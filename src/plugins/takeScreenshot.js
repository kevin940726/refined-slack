...
const addButtonToMessageActions = eventName => {
  ...
  const handleClick = messageContainer => {
    ...
    window.RSDispatchEvent(eventName, box);
  };
...
export default async (
  { addScriptToEvaluateOnNewDocument, addEventListener },
  Protocol
) => {
  await addEventListener(SCREENSHOT_EVENT_NAME, takeScreenshot(Protocol));

  await addScriptToEvaluateOnNewDocument(
    addButtonToMessageActions,
